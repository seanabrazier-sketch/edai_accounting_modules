...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        state_prevailing=self.all_input["state_to_prevailing_wages"]["Maine"]
        benefit_bol_array=[
            "Yes" if self.zone_type_1=="Pine Tree Development Zone" else "No",
            "-",
            "-",
            "Yes" if  promised_wage> state_prevailing else "No",
            "Yes" if promised_job>=5 else "No",
            "Yes"
        ]
        main_bol="Yes" if benefit_bol_array.count("Yes")==3 else "No"

        #main
        if main_bol=="Yes":

            array2=[promised_job*promised_wage*0.0648*0.8 for i in range(11)]
            array1=[promised_job*promised_wage*0.0648*0.3 for i in range(11)]
        else:
            array2=[0 for i in range(11)]
            array1=[0 for i in range(11)]

        if main_bol=="Yes" and benefit_bol_array[0]=="Yes":
            main_array=array2
        else:
            if main_bol=="Yes" and benefit_bol_array[0]=="No":
                main_array=array1
            else:
