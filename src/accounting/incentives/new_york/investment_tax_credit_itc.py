...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        scientific_bol="Yes" if self.promised_jobs>0.09 and self.project_category=="R&D Center" else "No"
        ## not yet define row 4191
        default_value=0
        manufacturing_bol="Yes" if self.promised_jobs>default_value and self.high_level=="Manufacturing" else "No"

        self.main_bol="Yes" if manufacturing_bol=="Yes" or scientific_bol=="Yes" else "No"

        sum_val=self.machine+self.construction
        array1=[max([0.04*(sum_val-350000000),0]) if manufacturing_bol=="Yes" else 0 for i in range(11)]
        array1[0]=0
        array2=[min([350000000,sum_val])*0.05 if manufacturing_bol=="Yes" else 0 for i in range(11)]
        array2[0]=0
        array3=[i+j for i,j in zip(array2,array1)]
        scientific_array=[sum_val*0.09 if scientific_bol=="Yes" else 0 for i in range(11)]
        scientific_array[0]=0
        if  manufacturing_bol=="Yes":
            main_array=array3
        else:
            if scientific_bol=="Yes":
                main_array=scientific_array
            else:
