...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        array_bol=[
            "Yes" if self.high_level=="Manufacutring" else "No",
            "Yes" if self.project_category=="R&D Center" else "No",
            "Yes" if self.irs=="Data processing, hosting, and related servies" else "No"
        ]
        self.main_bol="Yes" if array_bol.count("Yes")>0 else "No"
        ## this value link to florida --> Ask Sean why if it is correct then need to embed that value
        ## check row 4224
