...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        array_value=[1,0.9,0.75,0.9,0.9]
        choose_rate=look_up(tier,tier_array,array_value)

        geographic_bol=[
            "Yes" if i==tier else "No" for i in tier_array
        ]
        min_county_wage=[
            "Yes" if self.promised_wage>=prevailing else "No" for i in range(4)
        ]

        meet_bol=[
        "Yes" if geographic_bol[0]=="Yes" and min_county_wage[0]=="Yes" else "No",
            "Yes" if geographic_bol[1] == "Yes" and min_county_wage[1] == "Yes" else "No",
            "Yes" if geographic_bol[2] == "Yes" and min_county_wage[2] == "Yes" else "No",
            "Yes" if geographic_bol[3]=="Yes" and self.promised_jobs>=1750 and self.promised_cap>=500000000 else "No",
            "Yes" if geographic_bol[4] == "Yes" and self.promised_jobs >= 3000 and self.promised_cap >=1000000000 else "No",

        ]

        self.main_bol="Yes" if meet_bol.count("Yes")>0 else "No"

        year_array=[12,12,12,20,30]
        choose_year=look_up(tier,tier_array,year_array)

        main_array=[min([16000,0.0535*self.promised_wage*0.75])*self.promised_jobs for i in range(choose_year+1)]
        main_array[0]=0
