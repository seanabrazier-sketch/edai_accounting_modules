...
from util.npv import npv,excel_npv
...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def estimated_incentives(self)->List[float]:
        ## default 10 years including year 0
    ...
    def final_return(self):
        ...
        county_unemploy=None
        try:
            county_unemploy=list_of_special_localities["Unemployment, 2019"][self.county]
        except:
            county_unemploy=0
        ## converting to float
        ...
        try:
            county_unemploy=float(county_unemploy)
        except:
            county_unemploy=0


        state_unemploy=self.all_input["state_to_unemployment_rate"]["Ohio"]
        self.main_bol="Yes" if county_unemploy>=state_unemploy*1.25 else "No"

        if self.main_bol=="Yes":
            main_array=[i*0.6 for i in self.property_tax]
            main_array[0]=0
        else:
