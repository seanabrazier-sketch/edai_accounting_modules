...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        #main
        federal=self.federal_minimum_wage*1.75*52000
        main_bol_array=[
            "Yes" if self.promised_jobs>=10 else "No",
            "Yes" if self.equipvalent_payroll_base>=federal else 'No',
            "Yes" if self.promised_wage/2080>=1.5*7.25  else "No"
        ]
        self.main_bol="Yes" if main_bol_array.count("Yes")==3 else "No"

        array1=[i*0.45 for i in self.gross_receipts_tax]
        if self.main_bol=="Yes":
            main_array=array1
            main_array[0]=0
        else:
