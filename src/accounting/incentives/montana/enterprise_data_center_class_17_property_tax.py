...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        self.main_bol="Yes" if bol_array.count("Yes")==3 else "No"
        array1=[0.63 for i in range(6)]

        array1[0]=0
        for i in range(5):
            value=round(array1[-1]-0.13,2)
            array1.append(value)
        array1[-1]=0
        main_array=[]
        for i in range(11):
            if self.main_bol=="Yes":
                if i==0:
                    ...
                    main_array.append(self.property_tax[i]*array1[i])
                else:
                    main_array.append(self.property_tax[i]*array1[i-1])
            else:
                main_array.append(0)
