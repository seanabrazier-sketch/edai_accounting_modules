...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        if isinstance(self.county,str):
            prevailing=self.prevailing_wage_county[self.county.replace("MT","Montana")]
        else:
            prevailing=self.prevailing_state["Montana"]
        value_array=[
            8.65*1.7*2080,
            prevailing
        ]
        ...
        state_poverty=self.all_input["state_to_poverty_rate"]["Montana"]

        requirement_bol_array=[
            "Yes" if self.promised_wage>=min_val else "No",
            "Yes" if county_poverty>state_poverty else "No"
        ]


        ## benefit
        benefit_bol_array=[requirement_bol_array[1],"Yes" if requirement_bol_array[1]=="No" else "No"]
        array_value=[7500,5000]

        choose_value=look_up("Yes",benefit_bol_array,array_value)

        self.main_bol=requirement_bol_array[0]

        #main

        array1=[self.promised_jobs*choose_value if self.main_bol=="Yes" else 0 for i in range(11)]

        array1[0]=0

        array2=[i*0.08 for i in array1]

        main_array=[i - min([j,30000]) for i,j in zip(array1,array2)]
