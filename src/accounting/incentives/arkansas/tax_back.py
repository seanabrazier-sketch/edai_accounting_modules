...
# Embedded file name: C:\Users\tienn\PycharmProjects\pythonGit\edai_accounting_modules-Tien_Branch\src\accounting\incentives\arkansas\tax_back.py
...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        requirement_bol = [
         'Yes' if self.promised_cap >= 100000 else 'No',
         'Yes',
         'Yes',
         'Yes']
        sum_val = self.construction + self.machine
        sum_val = sum_val * self.state_local_sale_tax_rate
        self.main_bol = 'Yes' if (requirement_bol[0:2].count('Yes') == 2 and requirement_bol[2:4].count('Yes') == 2) else 'No'
        df_dict = defaultdict(list)
        for i in range(11):
            df_dict['year'].append(i)
            if self.main_bol == 'Yes':
                df_dict['value'].append(sum_val)
            else:
