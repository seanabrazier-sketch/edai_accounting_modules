...
# Embedded file name: C:\Users\tienn\PycharmProjects\pythonGit\edai_accounting_modules-Tien_Branch\src\accounting\incentives\arkansas\advantage_arkansas_income_tax_credit.py
...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        median_earning = list_of_special_localities()
        median_earning.index = median_earning['Postal'].tolist()
        median_earning = median_earning['Median Earnings']['AR'].tolist()
        median_earning[0] = 0
        median_earning = [float(x) for x in median_earning]
        min_val = min(median_earning)
        requirement_bol = 'Yes' if self.promised_wage >= min_val else 'No'
        array1 = [i * 0.5 for i in self.state_corporate_income_tax]
        array1[0] = 0
        array2 = [min([i, self.promised_jobs * self.promised_wage * 0.01]) for i in array1]
        array2[0] = 0
        tier = ['Tier 1', 'Tier 2', 'Tier 3', 'Tier 4']
        try:
            choose_tier = tier[self.county]
        except:
            choose_tier = 'Tier 1'

        tier_bol = [
         'Yes' if (self.equipvalent_payroll_base >= 125000 and choose_tier == 'Tier 1') else 'No',
         'Yes' if (self.equipvalent_payroll_base >= 100000 and choose_tier == 'Tier 2') else 'No',
         'Yes' if (self.equipvalent_payroll_base >= 750000 and choose_tier == 'Tier 3') else 'No',
         'Yes' if (self.equipvalent_payroll_base >= 50000 and choose_tier == 'Tier 4') else 'No']
        self.main_bol = 'Yes' if (tier_bol.count('Yes') > 0 and requirement_bol == 'Yes') else 'No'
        if self.main_bol == 'Yes':
            main_array = array2
        else:
