...
# Embedded file name: C:\Users\tienn\PycharmProjects\pythonGit\edai_accounting_modules-Tien_Branch\src\accounting\incentives\arkansas\create_rebate_cash_rebate.py
# Compiled at: 2022-01-13 11:55:50
# Size of source mod 2**32: 7167 bytes
...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        requirement_bol = 'Yes' if self.equipvalent_payroll_base >= 2000000 else 'No'
        self.main_bol = requirement_bol
        alternative_bol_array = [
         'Yes' if self.promised_wage >= 1.75 * prevailing_wage else 'No',
         'Yes' if self.equipvalent_payroll_base >= 250000 else 'No']
        alternative_bol = 'Yes' if alternative_bol_array.count('Yes') == 2 else 'No'
        year_to_use = 11 if alternative_bol == 'Yes' else 3
        array_value = [0.039, 0.0425, 0.045, 0.05]
        choose_val = look_up(choose_tier, tier, array_value)
        rate_array = [choose_val, 0.05 if alternative_bol == 'Yes' else 0]
        sum_rate = sum(rate_array)
        main_array = [self.promised_wage * self.promised_jobs * sum_rate if self.main_bol == 'Yes' else 0 for i in range(11)]
        main_array[0] = 0
        self.year_to_use = year_to_use
