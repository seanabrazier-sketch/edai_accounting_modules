...
# Embedded file name: C:\Users\tienn\PycharmProjects\pythonGit\edai_accounting_modules-Tien_Branch\src\accounting\incentives\arkansas\arkplus_income_tax_credit.py
...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        if self.equipvalent_payroll_base >= 2000000:
            tier_bol = ['Yes' if (choose_tier == 'Tier 1' and self.promised_cap >= 5000000) else 'No',
             'Yes' if (self.equipvalent_payroll_base >= 1500000 and choose_tier == 'Tier 2' and self.promised_cap >= 3750000) else 'No',
             'Yes' if (self.equipvalent_payroll_base >= 1200000 and choose_tier == 'Tier 3' and self.promised_cap >= 3000000) else 'No',
             'Yes' if (self.equipvalent_payroll_base >= 800000 and choose_tier == 'Tier 4' and self.promised_cap >= 2000000) else 'No']
            sum_val = self.land + self.construction + self.machine
            index_val = tier.index(choose_tier)
            self.main_bol = tier_bol[index_val]
            array1 = [i * 0.5 for i in self.state_corporate_income_tax]
            array1[0] = 0
            array2 = [min([i, sum_val * 0.1]) for i in array1]
            array2[0] = 0
            if self.main_bol == 'Yes':
                main_array = array2
        else:
