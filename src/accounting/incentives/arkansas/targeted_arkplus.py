...
# Embedded file name: C:\Users\tienn\PycharmProjects\pythonGit\edai_accounting_modules-Tien_Branch\src\accounting\incentives\arkansas\targeted_arkplus.py
# Compiled at: 2022-01-13 11:55:50
# Size of source mod 2**32: 7821 bytes
from accounting.incentives import *
import accounting.incentives.alabama.jobs_act_incentives_jobs as jobs
from collections import defaultdict
import util.npv as npv
...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.construction = self.capex.amount(industry_type=(self.pnl_input['industry_type']), property_type=(RealProperty.CONSTRUCTION_MATERIAL))
        self.machine = self.capex.amount(industry_type=(self.pnl_input['industry_type']), property_type=(PersonalProperty.MACHINERY_AND_EQUIPMENT))
        ...
        self.gross_receipts_tax = self.npv_dicts['Gross receipts tax']
        self.sale = self.npv_dicts['Sales']
    ...
    def final_return(self):
        ...
        if isinstance(self.county, str):
            prevailing_wage = self.prevailing_wage_county[self.county.replace('AR', 'Arkansas')]
        else:
            prevailing_wage = self.prevailing_state['Arkansas']
        state_prevailing = self.prevailing_state['Arkansas']
        prevailing_array = [state_prevailing, prevailing_wage]
        min_prevailing = min(prevailing_array)
        first_bol_array = [
         'Yes' if self.promised_wage >= min_prevailing * 1.75 else 'No',
         'Yes' if self.promised_wage >= min_prevailing * 2 else 'No',
         'Yes' if self.promised_wage >= min_prevailing * 2.25 else 'No']
        second_bol_array = [
         'Yes' if self.promised_cap >= 250000 else 'No',
         'Yes' if self.equipvalent_payroll_base >= 250000 else 'No',
         'Yes' if self.promised_wage >= min_prevailing * 1.75 else 'No']
        remainder_array = [
         min([max([0, self.promised_cap - 250000]), 250000]),
         min([max([0, self.promised_cap - 500000]), 500000]),
         min([max([0, self.promised_cap - 1000000]), 1000000]),
         min([max([0, self.promised_cap - 2000000]), 2000000])]

        benefit_array = [
         5000.0 if self.promised_cap > 500000 else 0.02 * remainder_array[0],
         0.04 * remainder_array[1],
         0.06 * remainder_array[2],
         0.08 * remainder_array[3]]
        self.main_bol = 'Yes' if (first_bol_array.count('Yes') > 0 or second_bol_array.count('Yes') == 3) else 'No'
        sum_benefit = sum(benefit_array)

        sum_mac_cons = (self.construction + self.machine) * self.state_local_sale_tax_rate
        main_array1 = [sum_mac_cons if self.main_bol == 'Yes' else 0 for i in range(11)]
        main_array2 = [sum_benefit if self.main_bol == 'Yes' else 0 for i in range(11)]

        main_array = [max([i, j]) for i, j in zip(main_array2, main_array1)]
        main_array[0] = 0
    ...
    def get_county_name(self):
        ...
        for i in county_list:
            try:
                index = i.index(', Arkansas')
                value = i.replace('Arkansas', 'AR')
                county.append(value)
                break
            except:
                try:
