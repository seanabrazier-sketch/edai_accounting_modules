...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        ## benefit
        benefit_bol_array=[
            "Yes" if self.promised_wage<=1.75*7.25*2080 else "No",
            "Yes" if self.promised_wage>7.25*1.75*2080 and self.promised_wage<=7.25*2.5*2080 else "No",
            "Yes" if self.promised_wage>=2.5*7.25*2080 else "No"
        ]
        array_value=[0.04,0.05,0.06]
        choose_val=look_up("Yes",benefit_bol_array,array_value)

        array_value2=[
            self.construction+self.machine+self.fix,
            self.promised_jobs*20000
        ]
        min_val=min(array_value2)

        array1=[min_val*0.04 if self.main_bol=="Yes" else 0 for i in range(11)]
        array1[0]=0
        array2=[self.promised_wage*self.promised_jobs*choose_val if self.main_bol=="Yes" else 0 for i in range(11) ]
        array2[0]=0
        main_array=[min([40000,j+k]) for j,k in zip (array1,array2)]
        main_array[0]=0

        df_dict=defaultdict(list)
        df_dict["year"]=self.default_year
        df_dict["value"]=main_array
        df_dict["4% on the lesser of the two"]=array1
        df_dict["Benefits(Sum)"]=array2
        return df_dict
