...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        self.main_bol="Yes" if self.zone_type_1=="Border County" else "No"

        ## distribution center Eligiility
        distribution_bol_array=["Yes" if self.project_category=="Distribution center" else "No",
                                "Yes",
                                "Yes"
                                ]
        #benefit
        benefit_bol_array=[
            "Yes" if distribution_bol_array.count("Yes")==3 else "No"]
        benefit_bol_array.append("Yes" if self.main_bol=="Yes" and benefit_bol_array[-1]=="No" else "No")
        if benefit_bol_array[-1]=="Yes":
            array1=[self.property_tax_rate*self.total_real_and_personal_prop for i in range(11)]
        else:
        ...
        if benefit_bol_array[0]=="Yes":
            array2=[i*0.5  for i in self.property_tax]
        else:
            array2=[0 for i in range(11)]
        array2[0]=0

        if self.main_bol=="Yes":
            main_array=[i+j for i,j in zip(array1,array2)]
        else:
