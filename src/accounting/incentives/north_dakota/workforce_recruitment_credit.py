...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def get_county_name(self):
        ...
        for i in county_list:
            try:
                index = i.index(", North Dakota")
                value = i.replace("North Dakota", "ND")
                county.append(value)
                break
            except:
                try:
    ...
    def final_return(self):
        ...
        if isinstance(self.county,str):
            prevailing_wage=self.project_level_inputs["Prevailing wages county"][self.county.replace("ND","North Dakota")]
        else:
            prevailing_wage=self.project_level_inputs["Prevailing wages"]["North Dakota"]
        ## main
        ...
        self.main_bol="Yes" if main_bol_array.count("Yes")==2 else "No"
        if self.main_bol=="Yes":

            main_array=[self.promised_jobs*self.promised_wage*0.05 for i in range(11)]
