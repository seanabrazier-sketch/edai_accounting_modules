...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def get_county_name(self):
        ...
        for i in county_list:
            try:
                index = i.index(", Missouri")
                value = i.replace("Missouri", "MO")
                county.append(value)
                break
            except:
                try:
    ...
    def final_return(self):
        ...
        if isinstance(self.county,str):
            prevailing=self.prevailing_wage_county[self.county.replace("MO","Missouri")]
        else:
            prevailing=self.prevailing_state["Missouri"]
        ## new facility
        new_facility_bol_array=[
            "Yes" if self.promised_jobs>=10 else "No",
            "Yes" if self.promised_wage>=prevailing*1.5 else "No",
            "Yes" if self.promised_cap>=25000000 else "No"
        ]

        new_facility_bol="Yes" if new_facility_bol_array.count('Yes')==3 else "No"
        ...
        expansion_bol_array=[
            ...
            "Yes" if self.promised_wage>=prevailing*1.5 else "No",
            "Yes" if self.promised_cap>=5000000 else "No"
        ]
        expansion_bol="Yes" if expansion_bol_array.count("Yes")==3 else "No"

        data_center_bol="Yes" if self.irs=="Data processing, hosting, and related services" else "No"
        bol1="Yes" if expansion_bol=="Yes" and self.attraction=="Expansion" else "No"
        bol2="Yes" if new_facility_bol=="Yes" and self.attraction=="Relocation" else "No"
        self.main_bol="Yes" if bol1=="Yes" and bol2=="Yes" else "No"

        if self.main_bol=="Yes":
            main_array=[self.state_local_sale_tax_rate*i for i in self.annual_exp]
            main_array[0]=self.state_local_sale_tax_rate*(self.construction+self.machine)
        else:
