...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        requirement_bol_array=[
            "Yes" if self.irs=="Professional, scientific, and technical services" else "No",
            "Yes" if self.promised_jobs>=500 else "No",
            "n/a",
        ...
        #option a
        option_a1=[
            self.promised_jobs*400 for i in range(11)

        ]
        option_a1[0]=0
        option_a2=[
            0.04*(self.land+self.machine+self.construction+self.fix) for i in range(11)
        ]
        option_a2[0]=0
        option_a3=[i+j for i,j in zip(option_a1,option_a2)]

        #option B
        option_b1=[self.promised_jobs*500 for i in range(11)]
        option_b1[0]=0
        option_b2=[1330000 for i in range(11)]
        option_b2[0]=0
        option_b3=[i+j for i,j in zip (option_b1,option_b2)]

        if self.main_bol=="Yes":
            main_array=[max([i,j]) for i,j in zip(option_a3,option_b3)]
        else:
