...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def get_county_name(self):
        ...
        for i in county_list:
            try:
                index = i.index(", New Mexico")
                value = i.replace("New Mexico", "NM")
                county.append(value)
                break
            except:
                try:
    ...
    def final_return(self):
        ...
        sector_bol_array=[
            ...
            "Yes" if self.high_level=="Transportation and warehousing" else "No",
        ...
        if isinstance(self.county,str):
            county_pop=float(list_of_special_localities()["Population"][self.county])
        else:
            county_pop=0

        ## minimum salary
        minimum_bol="Yes" if county_pop<60000 and self.promised_wage>=40000 else("Yes" if county_pop>=60000 and self.promised_wage>=60000 else "No")

        self.main_bol="Yes" if minimum_bol=="Yes" and other_requirement_bol_array.count("Yes")==2 else "No"

        main_array=[
            min([0.1*self.promised_wage,12000])*self.promised_jobs if self.main_bol=="Yes" else 0 for i in range(11)
        ]
