...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        self.main_bol = 'No'
        array1 = [self.state_local_sale_tax_rate * i for i in self.annual_exp]
        array1 = [self.state_local_sale_tax_rate * (self.machine + self.construction)]
        array2 = [0 for i in range(11)]
        array2[1] = 0.005 * (self.machine + self.construction)
        if self.main_bol == 'Yes':
            main_array = [i + k for i, k in zip(array1, array2)]
        else:
            mai_array = [0 for i in range(11)]
        df_dict = defaultdict(list)
        df_dict['year'] = self.default_year
        df_dict['value'] = mai_array
        return df_dict