...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        annual_exp=self.npv_dicts["Annual capital expenditures option 2"]
        #main_tab
        main_bol="Yes" if high_level_category=="Manufacturing" else "No"
        main_array=[]
        for i in range(11):
            if main_bol=="Yes":
                if i==0:
                    main_array.append(machinery*state_local_sales_tax_rate)
                else:
                    main_array.append(annual_exp[i]*state_local_sales_tax_rate)
            else:
                main_array.append(0)
