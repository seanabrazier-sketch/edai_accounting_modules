...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def estimated_eligibility(self) -> bool:
        if (self.rd_spending * self.sales_data) >= self.min_annual_spend:
            return True
        else:
            return False
    ...
    def estimated_incentives(self) -> List[float]:
        meets = [None] * 10
        meets = ['Yes' if (self.rd_spending * self.sales_data) >= self.min_annual_spend else 'No' for i in meets]
        ...
        incentives = []
        for i in nums4[:4]:
            incentives.append(i * self.benefit_starting)
        for i in nums4[4:]:
            incentives.append(i * self.benefit_expenses)
