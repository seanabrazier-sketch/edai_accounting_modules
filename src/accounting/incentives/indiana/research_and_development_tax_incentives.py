...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        self.main_bol="Yes" if self.project_category=="R&D Center" else "No"
        array1=self.rd[0:4]
        array1[0]=0
        array1=[i*0.05 for i in array1]
        array2=[]
        for i in range(1,8):
            val=(sum(self.rd[i:i+3])/3)*0.5*0.1
            array2.append(val)
        for i in array2:
            array1.append(i)


        array4=[self.state_local_sale_tax_rate*i if self.main_bol=="Yes" else 0 for i in self.annual_exp]
        array4[0]=self.total_personal_prop*self.state_local_sale_tax_rate if self.main_bol=="Yes" else 0

        main_array=[i+k for i,k in zip(array4,array1)]
