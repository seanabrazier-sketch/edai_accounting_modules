...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        ## need to inform with Sean Missing County Population lookup
        anual_exp[0]=0
        ## default population =0 for now
        county_population=0
        ##requiremnt
        reqirement_first_bol_array=[
            "Yes" if(county_population<50000 and promised_capital>=25000000)   else "No",
            "Yes" if (county_population<100000 and county_population>=50000 and promised_capital>=100000000) else "No",
            "Yes" if (county_population>=50000 and promised_capital>=15000000) else "No"
        ]

        reqirement_second_bol_array=[
            "Yes" if irs_sector=="Data processing, hosting, and related services" else "No",
            "Yes" if reqirement_first_bol_array.count("Yes")>0 else "No"
        ]

        #main tab

        main_bol="Yes" if  reqirement_second_bol_array.count("Yes")==2 else "No"
        self.main_bol  = main_bol
        local_option_bol="No"
        local_option_array=[i if local_option_bol=="Yes" else 0 for i in property_tax]
        local_option_array[0]=0
        main_array=[i*state_local_sales_tax if main_bol =="Yes" else 0 for i in anual_exp]
        if main_bol=="Yes":
            main_array[0]=state_local_sales_tax*machinery
        else:
            main_array[0]=0
