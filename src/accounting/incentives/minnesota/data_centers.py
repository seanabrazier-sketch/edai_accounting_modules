...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ## benefit
        array1=[self.total_real_and_personal_prop*self.property_tax_rate for i in range(11)]
        array1[0]=0

        ## expansion
        expansion_bol_array=[
            "Yes" if self.promised_cap>=50000000 else "No",
        "Yes"
        ]
        expansion_bol="Yes" if self.attraction=="Expansion" and expansion_bol_array.count("Yes")==2 else "No"

        ## requirement
        requirement_bol_array=[
            "Yes" if self.promised_cap>=30000000 else "No",
            "Yes"
        ]
        requirement_bol="Yes" if self.attraction=="Expansion" and requirement_bol_array.count("Yes")==2 else "No"


        ## main
        sector_bol="Yes" if self.irs=="Data processing, hosting, and related services" else "No"

        self.main_bol="Yes" if (requirement_bol=="Yes" or expansion_bol=="Yes") and sector_bol=="Yes" else "No"
        if self.main_bol=="Yes":

            array2=[self.state_local_sale_tax_rate*i for i in self.annual_exp]
            array2[0]=0
            main_array=[i+j for i,j in zip (array2,array1)]
        else:
