...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        capital_expenditure=self.npv_dicts["Annual capital expenditures option 2"]
        state_local_tax_rate=self.pnl_input["state_local_sales_tax_rate"]
        ## maintab
        bol_array=["Yes" if project_category=="Manufacturing" else "No", "Yes" if high_level_category=="Manufacturing" else "No"]
        main_bol="Yes" if bol_array.count("Yes")>0 else "No"
        for i in range(11):
            ...
            if main_bol=="No":
                ...
                if i==0:
                    df_dict["value"].append(machinery*state_local_tax_rate)
                else:
                    df_dict["value"].append(state_local_tax_rate*capital_expenditure[i])
