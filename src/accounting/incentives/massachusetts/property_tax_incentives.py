...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        self.main_bol="Yes" if high_level=="Manufacturing"  else "No"
        ...
        machine=self.capex.amount(industry_type=self.pnl_input["industry_type"],property_type=PersonalProperty.MACHINERY_AND_EQUIPMENT)
        fix=self.capex.amount(industry_type=self.pnl_input["industry_type"],property_type=PersonalProperty.FIXTURES)
        total=construction+machine+fix
        property_tax=self.pnl_input["property_tax_rate"]

        if self.main_bol=="Yes":
            main_array=[total*property_tax for i in range(11)]
        else:
