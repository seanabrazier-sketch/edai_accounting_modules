...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        sector_bol_array=[
            ...
            "Yes" if self.high_level=="Wholesale trade" else "No"
        ]
        sector_bol="Yes" if sector_bol_array.count("Yes")>0 else "No"
        ## benefit
        array1=[i*0.50 for i in self.state_corporate_income_tax]
        geography_array=[
            "Yes" if self.zone_type_2=="Tier 1 Developed" else "No",
            "Yes" if self.zone_type_2=="Tier 2 Moderately Developed" else "No",
            "Yes" if self.zone_type_2=="Tier 3 Less Developed" else "No"
        ]
        eligible_array=[
            "Yes" if geography_array[0]=="Yes" and self.promised_jobs>=20 else "No",
            "Yes" if geography_array[1]=="Yes" and self.promised_jobs>=20 else "No",
            "Yes" if geography_array[2]=="Yes" and self.promised_jobs>= 20 else "No"
        ]
        self.main_bol="Yes" if sector_bol=="Yes" and eligible_array.count("Yes")>0 else "No"
        benefit=[
            0.025* self.equipvalent_payroll_base if  eligible_array[0]=="Yes" else 0,
            0.05*self.equipvalent_payroll_base if eligible_array[1]=="Yes" else 0,
            0.1*self.equipvalent_payroll_base if eligible_array[2]=="Yes" else  0
        ]
        max_val=max(benefit)
        if self.main_bol=="Yes":
            main_array=[max_val for i in range(11)]
            main_array[0:2]=0
        else:
