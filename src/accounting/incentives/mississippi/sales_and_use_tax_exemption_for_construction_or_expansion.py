...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        sector_requirement_array=[
            ...
            "Yes" if self.project_category=="Corporate headquarters" else "No",
            "Yes" if self.high_level=="Wholesale trade" else "No",
            "Yes" if self.irs=="Data processing, hosting, and related services" else "No"
                                  ]
        sector_bol="Yes" if sector_requirement_array.count("Yes")>0 else "No"
        self.main_bol=sector_bol
        ...
        array_value=[0.5,0.5,1]
        choose_value=array_value[2] if tier_array[2]=="Yes" else "No eligible geographies"

        if self.main_bol=="Yes":
            if isinstance(choose_value,float):
                main_array=[choose_value*self.state_local_sale_tax_rate*(self.machine+self.construction) for i in range(11)]
            else:
