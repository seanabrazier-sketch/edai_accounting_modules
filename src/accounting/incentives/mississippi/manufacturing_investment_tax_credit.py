...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ...
        requirement_array=[
            ...
            "Yes" if self.construction+self.machine >=1000000 else "No"
        ]


        self.main_bol="Yes" if requirement_array.count("Yes")==3 else "No"
        if self.main_bol=="Yes":
            value=0.05*(self.machine+self.construction)
            main_array=[min([value,1000000]) for i in range(11)]
        else:
