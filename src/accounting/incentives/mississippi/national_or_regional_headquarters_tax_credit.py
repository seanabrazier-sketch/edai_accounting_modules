...
class IncentiveProgram(IncentiveProgramBase):
    ...
    def final_return(self):
        ## benefit
        prevailing_wage=self.project_level_inputs["Prevailing wages"]["Mississippi"]
        benefit_bol_array=[
            "Yes" if self.promised_wage>=prevailing_wage else "No",
            "Yes" if self.promised_wage >= prevailing_wage*1.25 else "No",
            "Yes" if self.promised_wage >= prevailing_wage*2 else "No",
        ...
        self.main_bol="Yes" if requirement_bol_array.count("Yes")==2 else "No"
        array_val=[500,1000,2000]
        choose_val=choose_with_condition("max","Yes",benefit_bol_array,array_val)
        ## missing condition check on row I3114
        ## default "No"
        bol="No"
        if self.main_bol=="Yes":
            main_array=[choose_val*self.promised_jobs for i in range(11)]
            if bol=="Yes":
                main_array[0]=self.state_local_sale_tax_rate*(self.machine+self.construction)
            else:
                main_array[0]=0
